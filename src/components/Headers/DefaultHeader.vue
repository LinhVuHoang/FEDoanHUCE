<template>
	...
	<a-layout-header >
		<div class="header-col header-brand" v-if="desiredPath =='ChangePass'">
		...
		<div class="header-col header-nav" v-if="desiredPath =='ChangePass'">
...
<script>
    ...
    methods:{
      getPath(){
        const fullPath = localStorage.getItem('link')
// Sử dụng hàm split('/') để tách fullPath thành một mảng các phần tử
        const pathSegments = fullPath.split('/');

// Lấy phần tử cuối cùng của mảng, nó sẽ là phần "/ChangePass"
        this.desiredPath = pathSegments[pathSegments.length - 1];
        console.log(this.desiredPath)
      }
    }
  })
